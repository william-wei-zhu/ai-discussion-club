...
export default async function DirectoryPage({ params, searchParams }: {
  ...
  return <div className="wrap page-content">
    ...
    {visible.length ? <div className="grid gap-4 md:grid-cols-2">{visible.map((member, index) => <article className="flex items-start gap-4 rounded-2xl border border-border bg-secondary p-5" key={`${member.name}-${index}`}>
      ...
      {member.photoUrl ? <img className="h-[72px] w-[72px] shrink-0 rounded-full object-cover" src={member.photoUrl} alt="" width="72" height="72" referrerPolicy="no-referrer" /> : <div className="flex h-[72px] w-[72px] shrink-0 items-center justify-center rounded-full bg-background font-medium text-primary" aria-hidden="true">{initials(member.name)}</div>}
      <div className="min-w-0"><h2 className="text-2xl break-words">{member.name}{member.isHost ? <span className="ml-2 inline-block rounded-full border border-border px-2 py-1 align-middle font-sans text-xs tracking-normal">Host</span> : null}</h2>{member.background ? <p className="mt-2 text-sm leading-6">{member.background}</p> : null}{member.linkedinUrl ? <a className="button secondary mt-4" href={member.linkedinUrl} target="_blank" rel="noreferrer">View LinkedIn</a> : null}</div>
