...
export function EventList({ events }: { events: PublicEvent[] }) {
  ...
  return <div className="event-list">{events.map(event => <article className="event-row" key={event.id}><Link className="event-artwork" href={`/events/${encodeURIComponent(event.id)}`} aria-label={`View ${event.name}`}>{event.coverUrl ? <Image className="event-thumbnail" src={event.coverUrl} alt="" width={240} height={240} sizes="(max-width: 700px) 96px, 180px"/> : <div className="event-date"><span>{eventDate(event, { month: 'short' })}</span><strong>{eventDate(event, { day: 'numeric' })}</strong></div>}</Link><div className="event-info"><p>{eventDate(event, { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit', timeZoneName: 'short' })}</p><h3><Link href={`/events/${encodeURIComponent(event.id)}`}>{event.name}</Link></h3>{event.location && <p className="location"><MapPin size={16}/>{event.location}</p>}</div><Link className="button secondary" href={`/events/${encodeURIComponent(event.id)}`}>View event</Link></article>)}</div>;
}
