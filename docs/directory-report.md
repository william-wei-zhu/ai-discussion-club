...
All preference mutations require an exact Origin match from `APP_ALLOWED_ORIGINS`. Localhost is accepted only outside production. Event consent changes are authorized again against server-side approved guest or host membership, so the client cannot opt into an unrelated event or change another attendee.
